...
{elseif $object.class_identifier|eq('openpa_consiglio_collaboration_comment')}
    ...
    <p>
        La presente per notificarLe l'inserimento di un nuovo intervento nella tematica di dicussione <a href="http://{social_pagedata('consiglio').site_url}/consiglio/collaboration/{$area.contentobject_id}/room-{$room.node_id}"><strong>{$room.name|wash()}</strong></a> nell'area collaborativa alla quale partecipa:
    </p>
    <p>
...
{elseif $object.class_identifier|eq('openpa_consiglio_collaboration_file')}
    ...
    <p>La presente per notificarLe l'inserimento del documento <em>{$object.name|wash()}</em> nella tematica di dicussione <a href="http://{social_pagedata('consiglio').site_url}/consiglio/collaboration/{$area.contentobject_id}/room-{$room.node_id}"><strong>{$room.name|wash()}</strong></a> nell'area collaborativa alla quale partecipa</p>
{/if}

<hr />
<p>
    Link per accedere all'area collabaorativa riservata:<br />
    <a href="http://{social_pagedata('consiglio').site_url}/consiglio/collaboration/">{social_pagedata('consiglio').site_url}/consiglio/collaboration</a>
</p>
