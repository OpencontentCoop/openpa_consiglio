...
<form class="form" action="{'consiglio/action'|ezurl(no)}">
...
{foreach $materie as $materia}
    <tr>
        ...
        {if $materie_like|contains($materia.object_id)}
            <button type="submit" name="RemoveMateriaLike" value="{$materia.object_id}" class="btn btn-danger"><i class="fa fa-times"></i></button>
        {else}
            <button type="submit" name="AddMateriaLike" value="{$materia.object_id}" class="btn btn-success"><i class="fa fa-check"></i></button>
        {/if}
        </td>
    </tr>
{/foreach}
