...
{def $seduta_can_modify = object_handler($seduta_object).gestione_sedute_consiglio.stuff.liquidata|not()}
...
{foreach $spese.SearchResult as $spesa}
    <tr>
        ...
        <td><a href="#" class="btn btn-danger btn-xs remove-spesa"
               data-url="{concat('consiglio/gettoni/',$interval,'/',$politico_object.id, '/remove_spesa/', $spesa.object.id )|ezurl(no)}"><i class="fa fa-trash"></i> </a> </td>
        {/if}
    </tr>
{/foreach}
    <tr>
