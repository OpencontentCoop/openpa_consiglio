    ...
    <div class="col-md-{if is_set( $post.object.data_map.internal_comments )}9{else}12{/if}">
        ...
        <div role="tabpanel">
            ...
            <ul class="nav nav-tabs" role="tablist">
                ...
                {/foreach}
                {if and($post.current_state.identifier|eq('online'), count($post.punti)|gt(0))}
                    <li role="presentation">
                        <a href="#punti" aria-controls="punti"
                           role="tab" data-toggle="tab">Ordini del giorno</a>
                    </li>
                {/if}
            </ul>
            ...
            <div class="tab-content">
                ...
                {/foreach}
                {if and($post.current_state.identifier|eq('online'), count($post.punti)|gt(0))}
                <div role="tabpanel" class="tab-pane" id="punti">
                    {include uri='design:editorialstuff/proposta/parts/punti.tpl' post=$post}
                </div>
                {/if}
            </div>
